...
def get_prompt_construction_fn(prompt_type):

    few_shot_prompt = []
    def prompt_construction_fn(row):
        curr_prompt_type = prompt_type

        question = row['question']
        if curr_prompt_type == "baseline":
        ...
        elif curr_prompt_type == "oracle_baseline":
            prompt = "Answer the given question. You must conduct reasoning between <think> and </think> " +\
            ...
            raise NotImplementedError('RLHFDataset prompt template not implemented yet')

        structured_prompt = few_shot_prompt + [{"role" : "user", "content" : prompt}]
        return np.array(structured_prompt)

    return prompt_construction_fn
...
class RLHFDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        # Construct the prompts (according to your different settings)
        prompt_construction_fn = get_prompt_construction_fn(self.prompt_type)
        self.dataframe['prompt'] = self.dataframe.apply(prompt_construction_fn, axis=1)
