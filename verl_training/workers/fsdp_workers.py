...
from verl_training.workers.fsdp_checkpoint_manager import FSDPCheckpointManager
...
class AgenticActorRolloutRefWorker(Worker):
    """
    ...
    def init_model(self):
        from verl_training.workers.dp_actor import AgenticDataParallelPPOActor
        ...
        if self._is_actor:
            ...
            self.actor = AgenticDataParallelPPOActor(config=self.config.actor,
        ...
        if self._is_ref:
            ...
            self.ref_policy = AgenticDataParallelPPOActor(config=self.config.ref, actor_module=self.ref_module_fsdp)
...
n
    ...
    def _build_rollout(self):
        ...
        from verl_training.workers.vllm_rollout import vLLMRollout
        from verl_training.workers.fsdp_vllm import FSDPVLLMShardingManager # TODO: PEFT
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(self, path, del_local_after_load=False, only_load_params=False):
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
